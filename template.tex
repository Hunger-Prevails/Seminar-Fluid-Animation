...
\begin{algorithm}
	...
	\Fn{\FMain{particle $i$}}{
		...
		\;
		compute viscosity force $ F_{i}^{viscosity}(t_{i}) $ incurred by particle $i$ using Eq. \eqref{eq2}\;
		compute external forces $ F_{i}^{ext}(t_{i}) $ incurred by particle $i$\;
		compute advection force $ F_{i}^{*}(t_{i}) $ incurred by particle $i$\;
		compute advection velocity $ v_{i}^{*}(t_{i}) $ of particle $i$ using Eq. \eqref{eq3}\;
		...
		\;
		compute pressure force $ F_{i}^{pressure}(t_{i}) $ incurred by particle $i$ using Eq. \eqref{eq5}\;
		\;
